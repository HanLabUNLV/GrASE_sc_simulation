...
# the SINGLE shared moderator, and join z_mod back onto every row of its event.
if (disp == "EB") {
  gd <- group_by_event(sc, "diff", "n")
  cat(sprintf("[run_model] EB: estimating dispersion for %d events...\n", length(gd)))
  phis <- bind_rows(mclapply(gd, function(dd)
    tryCatch(estimator(dd), error = function(e) NULL), mc.cores = opt$mc_cores))
  ...
  sc <- sc %>% left_join(phi_table[, c("gene", "event", "z_mod")],
                         by = c("gene", "event"))
  sc$z_mod[is.na(sc$z_mod)] <- fallback_z
  cat(sprintf("[run_model] EB: %d/%d events moderated (fallback z_mod = %.3f)\n",
